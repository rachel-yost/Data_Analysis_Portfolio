...
fit of our model. The model produced an AUC of on our training set and
on our test set. The variables selected by Lasso were Varenicline, an
...
observations and included information on treatment assignment and the
following 21 additional covariates: age, sex, an indicator for
non-Hispanic white, indicator for Black, income, education, FTCD score
at baseline, smoking within 5 minutes of waking up, BDI score at
baseline, cigarettes per day at baseline, cigarette reward value at
baseline, Pleasant Events Schedule at baseline-substitute reinforcers,
Pleasant Events Schedule at baseline-complimentary reinforcers,
anhedonia, other lifetime DSM-5 diagnoses, taking antidepressant
medication at baseline, current vs past MDD, nicotine metabolism
ratio, indicator for using exclusively metholated cigarettes, and and
...
to quit smoking. We split the data into a training and test set
(70%/30%), randomly sampling within each of the four treatment
options. This resulted in 211 training observations and 89 test
observations. We performed multiple imputation on each set separately
...
miced_data_train <- mice(data[training_rows,], m=5, predictorMatrix = predmat)
miced_data_test <- mice(data[-training_rows,], m=5, predictorMatrix = predmat)
```
...
data_for_tbl <- data %>%mutate(group = case_when(Var == 0 & BA == 0 ~ "Placebo and Standard Treatment",
                           Var== 0 & BA == 1 ~ "Placebo and BA",
                           Var== 1 & BA == 0 ~ "Varenicline and Standard Treatment",
                           Var== 1 & BA == 1 ~ "Varenicline and BA"))
...
had skewed distributions. After evaluating square root, squared, and
...
anhedonia (r = .41), income with education (r = .40), FTCD score at
baseline with smoking within 5 minutes of waking up (r = .63), and
cigarettes per day with FTCD score (r = .52). Varenicline had a
...
p9 <- ggplot(anhendonia_BA_plot_df, aes(x=as.factor(abst), y = mean_score, fill= BA)) +
  ...
  geom_errorbar(aes(ymin = mean_score - 1.96 * SE, ymax = mean_score + 1.96 * SE), 
                position = position_dodge(width = 0.9), width = 0.25) +
  labs(y = "Mean Anhedonia Score", fill = "Treatment", x = "Smoking Abstinence", title = "f") +
  theme_minimal()
...
considerations as predictors/interaction terms, and we would like to
...
the sum of the squared residuals plus a tuning parameter multiplied by the $\ell_{1}$ norm of the coefficient vector $\beta$. Thus, the penalty term is $\lambda\sum^{p}_{j=1}|\beta_j|$. This $\ell_{1}$ penalty allows the coefficients to be shrunk to exactly zero, in contrast to the penalty used by ridge regression, $\lambda\sum^{p}_{j=1}\beta_j^2$, which shrinks the coefficients towards zero, but they do not reach zero. Best subset selection is another method to consider for variable selection. Best subset selection works by testing each possible combination of $p$ predictors and identifying the best subset of variables based on certain critia such as cross-validated predicted error, AIC, BIC, or adjusted $R^2$. However, since the number of possible models increases rapidly as the number of possible variables increases, it can become computationally difficult with large $p$. Since we have many variables and interactions to test, we chose to use Lasso instead.   


Within each of the 5
imputed data sets we fit the full model including BA, Varenicline and
...
indicator for if the participant is taking antidepressants. We then
...
results$Coefficients <- round(results$Coefficients, digits = 5)

#rownames(results) <- c("(Intercept)", "Behavioral Activation", "Varenicline", "Non-Hispanic White", "FTCD score", "Anhedonia", "Other Diagnosis", "Current MDD", "Nicotine Metabolite Ratio", "Only Menthol", "Readiness", "Behavioral Activation:Hispanic", "Behavioral Activation:Only Menthol")
